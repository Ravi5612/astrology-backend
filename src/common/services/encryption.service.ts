...
export class EncryptionService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!encryptionKey || encryptionKey.length !== 64) {
      this.logger.error('CRITICAL: ENCRYPTION_KEY must be a 64-character hex string (32 bytes)');
      // We don't throw here to prevent the whole app from crashing if the key is missing during dev
      // but we log it as critical.
    }
    this.key = encryptionKey ? Buffer.from(encryptionKey, 'hex') : Buffer.alloc(32);
  }
  ...
  encrypt(text: string | null | undefined): string | undefined {
    ...
    try {
      ...
      this.logger.error(`Encryption failed for text: ${text.substring(0, 3)}...`, error.stack);
  ...
  decrypt(encryptedText: string | null | undefined): string | undefined {
    ...
    try {
      ...
      this.logger.warn(`Decryption failed for format-matching string. Possible key mismatch.`);
