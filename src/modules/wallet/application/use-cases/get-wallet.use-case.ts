...
export class GetWalletUseCase {
  ...
  async execute(userId: string): Promise<Wallet> {
    ...
    const expert = await this.walletRepository.manager.findOne(ProfileExpert, {
    ...
    if (!walletOwnerId) {
      const merchant = await this.walletRepository.manager.findOne(
        ProfileMerchant,
        { where: { user: { id: userId } } },
      );
      if (merchant) {
    ...
    if (!walletOwnerId) {
      const agent = await this.walletRepository.manager.findOne(ProfileAgent, {
        where: { user: { id: userId } },
      });
      if (agent) {
    ...
    if (!walletOwnerId) {
      const client = await this.walletRepository.manager.findOne(
        ProfileClient,
        { where: { user: { id: userId } } },
      );
      if (client) {
