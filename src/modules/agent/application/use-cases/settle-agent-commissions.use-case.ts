...
export class SettleAgentCommissionsUseCase {
    ...
    async execute(userId: string) {
        return this.databaseService.transaction(async (queryRunner) => {
            ...
            const clientCommPercent = getSettingValue(['COMMISSION_FROM_CLIENT', 'COMMISSION_FROM_CLIENT'], 3);
            const expertCommPercent = getSettingValue(['COMMISSION_FROM_ASTROLOGER', 'COMMISSION_FROM_ASTROLOGER'], 3);
            ...
            if (amountToSettle <= 0) {
                return { success: true, message: 'All commissions already settled', settled_amount: 0 };
            }
