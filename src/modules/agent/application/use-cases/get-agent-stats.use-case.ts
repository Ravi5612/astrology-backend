...
export class GetAgentStatsUseCase {
    ...
    async execute(userId: string, range: string = '30d', dateRangeDto?: DateRangeDto) {
        ...
        try {
            ...
            expertEarnings = roleStats.find((r: any) => r.role_name === 'expert')?.total_comm || 0;

            const withdrawalStats = await this.walletFacade.getWithdrawalsStatus(userId);
            ...
            return {
                ...
                processing_withdrawals: withdrawalStats.processing_amount + withdrawalStats.approved_amount,
                ...
                total_listings_earnings: Number((expertEarnings + shopEarnings + mandirEarnings).toFixed(2)),
                success_rate: totalUsersCount > 0 ? ((usersWithActivity / totalUsersCount) * 100).toFixed(1) + "%" : "0%",
                rank: agentRank,
                revenue_growth: revenueGrowthRaw.length > 0 ? revenueGrowthRaw : [
                    { name: "No Data", value: 0 }
                ],
                registration_activity: registrationActivityRaw.map((r: any) => ({
                    day: r.day,
