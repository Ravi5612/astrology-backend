...
export class GetAdminAgentsUseCase {
  ...
  async execute(params: {
    ...
    const { items: users, total } = await this.usersFacade.findAllByRole('agent', params.search, page, limit, params.status);

    // Fetch commission settings
    const settings = await this.adminFacade.getSystemSettings(['COMMISION_FROM_CLIENT', 'COMMISION_FROM_ASTROLOGER']);
    ...
    const agentsData = await Promise.all(users.map(async (uObj) => {
      ...
      const profile = await this.agentFacade.getProfile(u.id);
      ...
      return {
        ...
        commission_rate: Number(profile?.commission_rate) || 10.00,
